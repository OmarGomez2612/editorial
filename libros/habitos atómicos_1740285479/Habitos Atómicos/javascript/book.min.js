...
void 0!=tmpBookConfig[d]&&(tmpBookConfig[d]=!1)}}catch(e){}recordError();c=Module._getVerifyString();d=UTF8ToString(c);Module._free(c);eval(d)};function recordError(){try{tmpBookConfig.largePageWidth&&tmpBookConfig.largePageHeight&&tmpBookConfig.totalPageCount||BookEvent.trigger("errorLog",[JSON.stringify({largePageWidth:tmpBookConfig.largePageWidth,largePageHeight:tmpBookConfig.largePageHeight,totalPageCount:tmpBookConfig.totalPageCount,type:"BookConfigError"})])}catch(b){}}
...
var VERSION_NAME="6.4.9",BUILD_DATE="2025020601",BookType={normal_book:0,single_book:1,slide_book:3,singleSlide_book:6,catalog_book:7,singleCatalog_book:8},normalBookStatu={book_statu_ready:0,book_statu_auto_flip:1,book_statu_mouse_flip:2,book_statu_mag_flip:3},singleBookStatu={book_statu_ready:0,book_statu_auto_flip:1,book_statu_mouse_flip:2,book_statu_mag_flip:3},bookCorner={top_left:0,top_right:1,bottom_left:2,bottom_right:3},BrowserType={WEBKIT:1,OPERA:1,MOZ:2,MS:3},Direction={left:"left",right:"right",
...
!1}if(!c&&b.x>this.MiddleX||c&&0<=c.indexOf("r")){if(this.flipping&&"right"==this.flipToDirection)return;d=this.realIndex(!0)[0];e=this.realIndex(!0)[1];f=d+"-"+e;g=!0}if(f&&d&&e){var h=originTotalPageCount;bookConfig.SingleModeBanFlipToLastPage&&this.bookType==BookType.single_book&&(h=Math.max(1,bookConfig.totalPageCount-1));if(!(0>=d||d>h||0>=e||e>h||this.flipPageManager[f]&&this.flipPageManager[f].fallback||this.flipPageManager[f]&&this.flipPageManager[f].flipping||BookEvent.trigger("beforeFlipPage",
[e])))if(h=e,BookInfo.isDoublePage()&&(h=BookInfo.getMinPageIndex(h)),passwardType()&&bookConfig.excludeFrontPages&&h>=parseInt(bookConfig.excludeFrontPages)&&!verifyPassward())global.passwardPannel||(global.passwardPannel=new PasswardPannel),global.passwardPannel.show();else return this.defineFlipPage(f,b,c,d,e,g),f}},defineFlipPage:function(b,c,d,e,f,g){if(c){var h=originTotalPageCount;bookConfig.SingleModeBanFlipToLastPage&&this.bookType==BookType.single_book&&(h=Math.max(1,bookConfig.totalPageCount-
1));0>=e||e>h||0>=f||f>h||(this.flipPageManager[b]?d=this.getFromStartPointByCoordinate(c,d,this.flipPageManager[b]):(this.beforeFlipTo(f),d=this.getFromStartPointByCoordinate(c,d,this.flipPageManager[b]),this.flipIntervalManager.flipToPoint(this.flipPageManager[b],e,f,d[0],d[1],!0)),g?(this.flipPageManager[b].AliceToLeft=!0,this.flipPageManager[b].AliceToRight=!1):(this.flipPageManager[b].AliceToLeft=!1,this.flipPageManager[b].AliceToRight=!0),this.flipPageManager[b].loosen=!1,this.flipPageManager[b].fromPoint=
d[0],this.flipPageManager[b].startPoint=d[1],this.flipPageManager[b].toPoint=c,0>this.flipPageManager.pinDownCurlingArray.indexOf(b)&&this.flipPageManager.pinDownCurlingArray.push(b),this.flipPageManager.fallbackArray.removeElement(b),this.flipPageManager.flipArray.removeElement(b))}},unCurling:function(){if(!(this.selected||this.status_zoom||this.status_resize)&&this.flipPageManager.array)for(var b=this.flipPageManager.array.length,c=0;c<b;c++){var d=this.flipPageManager.array[c];if(d!=this.optionPage&&
this.flipPageManager[d]&&!this.flipPageManager[d].fallback&&!this.flipPageManager[d].flipping&&(this.flipPageManager[d].AliceToRight||this.flipPageManager[d].AliceToLeft)){this.flipPageManager[d].AliceToRight=!1;this.flipPageManager[d].AliceToLeft=!1;this.flipPageManager[d].fallback=!0;var e=this.getRealToPoint(this.flipPageManager[d],this.flipPageManager[d].fromPoint,this.flipPageManager[d].startPoint);this.flipPageManager[d].toPoint=e.toPoint;this.flipPageManager[d].fromPoint=e.fromPoint;this.flipPageManager[d].coordinate=
e.coordinate;0>this.flipPageManager.fallbackArray.indexOf(d)&&this.flipPageManager.fallbackArray.push(d);this.flipPageManager.pinDownCurlingArray.removeElement(d);this.flipPageManager.flipArray.removeElement(d)}}},getFromStartPointByCoordinate:function(b,c,d){if(b){var e={x:0,y:0},f={x:0,y:0};if(bookConfig.FlipDirection){var g=d.getPageWidthHeight().height;d=d.getPageWidthHeight().width}else g=d.getPageWidthHeight().width,d=d.getPageWidthHeight().height;var h=b.y>d/2?b.y-5:b.y+5;0>b.x&&(e={x:-g,y:h});
0<b.x&&(e={x:g,y:h});c&&0<=c.indexOf("r")&&(e.x=g);c&&0<=c.indexOf("l")&&(e.x=-g);"tl"==c&&(e={x:-g,y:0});"bl"==c&&(e={x:-g,y:d});"tr"==c&&(e={x:g,y:0});"br"==c&&(e={x:g,y:d});f.x=e.x;f.y=e.y;return[e,f]}},getRealToPoint:function(b,c,d){if(bookConfig.FlipDirection){b.getPageWidthHeight();var e=b.getPageWidthHeight().width}else b.getPageWidthHeight(),e=b.getPageWidthHeight().height;if(0==c.y||c.y==e)return{fromPoint:c,toPoint:d,coordinate:b.coordinate};var f={},g={},h=b.coordinate;b.coordinate.y<=
c.y?(f.y=e,f.x=c.x,g.y=e):(f.y=0,f.x=c.x,g.y=0);g.x=d.x;b=(h.y-c.y)/(h.x-c.x);h.x=[(b*b-1)*f.x+b*(-2*f.y+c.y+h.y)+(c.x+h.x)]/(b*b+1);h.y=f.y+b*(h.x-f.x);return{fromPoint:f,toPoint:g,coordinate:h}},realIndex:function(b){var c=BookInfo.getMinPageIndex(this.currentPageIndex);if(b&&!rightToLeft||!b&&rightToLeft)return[c+1,c+2];if(!b&&!rightToLeft||b&&rightToLeft)return[c,c-1]},getNextPage:function(b){"right"==this.flipToDirection&&!rightToLeft||"left"==this.flipToDirection&&rightToLeft?previousPageFun(b):
nextPageFun(b)},realBookPosition:function(){if(bookConfig.upsideOnMobile())var b=this.flipBook.offset().top,c=this.flipBook.offset().left;else b=this.flipBook.offset().left,c=this.flipBook.offset().top;return[b,c]},realPoint:function(b){if((b=b.originalEvent&&b.originalEvent.changedTouches?b.originalEvent.changedTouches:b.changedTouches?b.changedTouches:[b])&&!(3<=b.length)){var c=this.realBookPosition()[0],d=this.realBookPosition()[1],e=b[0].pageX,f=b[0].pageY;bookConfig.upsideOnMobile()&&(e=b[0].pageY,
f=windowHeight-b[0].pageX);if(bookConfig.FlipDirection){f=f-d-this.pageHeight;var g=this.getCurrentPageWidthHeight(f),h=g.pageWidth;e=this.pageWidth-(e-c)-(this.pageWidth-h)/2;if(!b[1])return[{x:f,y:e}];g=b[1].pageX;b=b[1].pageY;g=this.pageWidth-(g-c)-(this.pageWidth-h)/2;b=b-d-this.pageHeight;return[{x:f,y:e},{x:b,y:g}]}e=e-c-this.pageWidth;g=this.getCurrentPageWidthHeight(e);h=g.pageHeight;f=f-d-(this.pageHeight-h)/2;if(!b[1])return[{x:e,y:f}];g=b[1].pageX;b=b[1].pageY;g=g-c-this.pageWidth;b=b-
d-(this.pageHeight-h)/2;return[{x:e,y:f},{x:g,y:b}]}},mouseWheelFlip:function(){var b;this.flipBook.bind(_event._mousewheel,function(c){if(void 0!=b&&150>(new Date).getTime()-b)var d=!1;else b=(new Date).getTime(),d=!0;if(!d)return!1;d=c.originalEvent;d=Math.max(-1,Math.min(1,d.wheelDelta||-d.detail));c=c.originalEvent?c.originalEvent:c;this.status_zoom&&(c={pointers:[{pageX:c.pageX,pageY:c.pageY},{pageX:c.pageX,pageY:c.pageY}]},0>d?this.controlBar&&this.controlBar.zoomDown(c):this.controlBar&&this.controlBar.zoomUp(c));
this.status_zoom||!bookConfig.mouseWheelFlip||this.flipping||(0>d?nextPageFun("mouse wheel flip"):previousPageFun("mouse wheel flip"));if(bookConfig.mouseWheelFlip)return stopEvent(c),!1}.bind(this))},mouseZoomEvent:function(){$(this.flipBook).bind(_event._move,function(b){var c=BookInfo.getBook();if(c.status_zoom&&!this.flipBook.parameters.zoomming&&c.controlBar&&c.controlBar.moveByMousePosition){c=this.flipBook;var d=c.width(),e=c.height(),f=b.pageX;b=b.pageY;d=f-f*d/windowWidth;e=b-b*e/windowHeight;
b={};b.origin=c.parameters.origin;b.scale=c.parameters.scale;d-=c.parameters.left;e-=c.parameters.top;b.tran={x:d,y:e};e=transformCSS(b);c.css(e);this.bookAttachments&&this.bookAttachments.zoomMap&&this.bookAttachments.zoomMap.refreshArea()}}.bind(this))}});
